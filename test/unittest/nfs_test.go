...
func TestNfsWithPVCCreation(t *testing.T) {
	t.Parallel()

	tfOptions := GenerateTFOptions(NfsPVCCreation, t, nfsModule)
	plan := terraform.InitAndPlanAndShowWithStruct(t, tfOptions)
	...
	terraform.RequirePlannedValuesMapKeyExists(t, plan, "kubernetes_persistent_volume.shared-home-pv[0]")
	terraform.RequirePlannedValuesMapKeyExists(t, plan, "kubernetes_persistent_volume_claim.shared-home-pvc[0]")
}
