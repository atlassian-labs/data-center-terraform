...
enable_ssh_tcp_protocol_on_lb_listener() {
  ...
  if [ -n "${install_bitbucket}" ]; then
    ...
    original_instance_port=$(aws elb describe-load-balancers --load-balancer-name ${load_balancer_name} --query 'LoadBalancerDescriptions[*].ListenerDescriptions[?Listener.LoadBalancerPort==`7999`].Listener[].InstancePort | [0]' --region "${region}")
