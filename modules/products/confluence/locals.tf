locals {
  ...
  db_master_username = var.db_master_username == null ? var.rds.rds_master_username : var.db_master_username
  db_master_password = var.db_master_password == null ? var.rds.rds_master_password : var.db_master_password
  cmd_psql_update    = "BASE_URL_TO_REPLACE=$(PGPASSWORD=${local.db_master_password} psql postgresql://${var.rds.rds_endpoint}/${local.product_name} -U ${local.db_master_username} -Atc \"select BANDANAVALUE from BANDANA where BANDANACONTEXT = '_GLOBAL' and BANDANAKEY = 'atlassian.confluence.settings';\" | grep -i '<baseurl>'); PGPASSWORD=${local.db_master_password} psql postgresql://${var.rds.rds_endpoint}/${local.product_name} -U ${local.db_master_username} -c \"update BANDANA set BANDANAVALUE = replace(BANDANAVALUE, '$${BASE_URL_TO_REPLACE}', '<baseUrl>${local.confluence_ingress_url}</baseUrl>') where BANDANACONTEXT = '_GLOBAL' and BANDANAKEY = 'atlassian.confluence.settings';\""
