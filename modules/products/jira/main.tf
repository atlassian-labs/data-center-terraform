...
resource "kubernetes_namespace" "jira" {
  metadata {
    name = local.product_name
  }
}

resource "kubernetes_persistent_volume" "atlassian-dc-jira-share-home-pv" {
  metadata {
    name = "atlassian-dc-jira-share-home-pv"
  }
  spec {
    capacity = {
...
resource "kubernetes_persistent_volume_claim" "atlassian-dc-jira-share-home-pvc" {
  metadata {
    name      = "atlassian-dc-jira-share-home-pvc"
  ...
  spec {
    ...
    volume_name        = kubernetes_persistent_volume.atlassian-dc-jira-share-home-pv.metadata[0].name
    storage_class_name = local.storage_class_name
  }
}
