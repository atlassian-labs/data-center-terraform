    ...
    Statement : [
      {
        Effect : "Allow",
        Principal : {
          Federated : "arn:aws:iam::${data.aws_caller_identity.current.account_id}:oidc-provider/${var.oicd_provider}"
        },
        Action : "sts:AssumeRoleWithWebIdentity",
        Condition : {
          StringEquals : {
            "${var.oicd_provider}:aud" : "sts.amazonaws.com",
            "${var.oicd_provider}:sub" : "system:serviceaccount:${local.lb_controller_namespace}:${local.lb_controller_service_account_name}"
          }
        }
      }
    ]
