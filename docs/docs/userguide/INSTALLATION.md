...
!!! help "How to run the product after installation?"    
    When the installation process finishes successfully, you can find some detailed information about the infrastructure in the console, including the `<product_url>`/`<load_balancer_hostname>` endpoint URL to launch the product in the browser and more.      
