...
??? hint "How do I deal with "Module not installed" error in installation or uninstallation?"

    Terraform needs some modules to process deployment and cleanup the infrastructure. 
    Those files are generated by terraform during install process and are stored in `.terraform` folder. 
    If terraform cannot find the module files, then it is not able to perform the required commands. 
    ...
    **Solution**
    ...
    ```
    2. re-run the install/uninstall script.