...
# -x : cleanup terraform state files generated by terraform.
...
EOF
  ...
  echo "   -x : cleanup terraform state files generated by terraform."
  echo "   -r <path_root_repo>: define path to root folder of repo."
  echo "      If '-r' is not provided it calculated it based on script path"
...
cleanup_local_terraform_state() {
  if [ ! -z "${CLEAN_TERRAFORM_STATE}" ]; then
    log "Cleaning terraform state files."
